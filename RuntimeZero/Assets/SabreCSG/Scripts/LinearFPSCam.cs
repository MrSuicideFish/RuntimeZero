...
namespace Sabresaurus.SabreCSG
{
	...
	//            }
	            ...
	            if(e.type == EventType.KeyDown)
	            {
					...
					if(KeyMappings.EventsMatch(e, EditorKeyMappings.GetViewForwardMapping(), true))
	                {
	                    forwardSpeed = 1;
	                }
					else if(KeyMappings.EventsMatch(e, EditorKeyMappings.GetViewBackMapping(), true))
	                {
	                    forwardSpeed = -1;
	                }

					if(KeyMappings.EventsMatch(e, EditorKeyMappings.GetViewStrafeRightMapping(), true))
	                {
	                    rightSpeed = 1;
	                }
					else if(KeyMappings.EventsMatch(e, EditorKeyMappings.GetViewStrafeLeftMapping(), true))
	                {
	                    rightSpeed = -1;
	                }
	              
					if(KeyMappings.EventsMatch(e, EditorKeyMappings.GetViewStrafeUpMapping(), true))
	                {
	                    upSpeed = 1;
	                }
					else if(KeyMappings.EventsMatch(e, EditorKeyMappings.GetViewStrafeDownMapping(), true))
	                {
	                    upSpeed = -1;
	                }
					e.Use();
