	...
	//					Undo.RecordObject(targetBrushTransform, "Rotated brush(es)");
		...
		void OnMouseUp (SceneView sceneView, Event e)
		{
			...
			else if(e.button == 0 && SabreInput.IsModifier(e, EventModifiers.Shift | EventModifiers.Control))
			{
 			...
 			else if(e.button == 0 && !SabreInput.AnyModifiersSet(e) && copyMaterialHeld) // Right click, copy material
			{
				if(!EditorHelper.IsMousePositionInIMGUIRect(e.mousePosition, ToolbarRect))
				{
					Ray ray = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);
					...
					if(polygon != null)
					{
						...
						if(sourcePolygon != null && sourcePolygon.Material != lastMaterial)
						{
							...
							ChangePolygonMaterial(sourcePolygon, lastMaterial, Color.white);
						}
					}
				}
			}
		...
		void FollowLastFace(Polygon sourceTargetPolygon)
		{
			...
			for (int sourceIndex = 0; sourceIndex < sourceBuiltPolygons.Length; sourceIndex++) 
			{
				for (int targetIndex = 0; targetIndex < targetBuiltPolygons.Length; targetIndex++) 
				{
					...
					if(SurfaceUtility.FindSharedEdge(sourceBuiltPolygons[sourceIndex], targetBuiltPolygons[targetIndex], out matchedEdge1, out matchedEdge2))
					{
...
//						VisualDebug.AddPoint(sourcePosition);
						...
						ChangePolygonMaterial(sourceTargetPolygon, lastMaterial, Color.white);
			...
			if(DragAndDrop.objectReferences.Length == 1 && DragAndDrop.objectReferences[0] is Material)
			{
				if(selectedSourcePolygons.Count > 0)
				{
					...
					for (int i = 0; i < selectedSourcePolygons.Count; i++) 
					{
						if(selectedSourcePolygons[i] != null)
						{
							ChangePolygonMaterial(selectedSourcePolygons[i], material, Color.white);
						}
					}
		...
		{
			if (KeyMappings.EventsMatch(e, Event.KeyboardEvent(KeyMappings.CopyMaterial)))
			{
...
//			GUILayout.Box(secondaryTexture, style, GUILayout.Width(width), GUILayout.Height(width));
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				Material drawMaterial = SabreGraphics.GetPreviewMaterial();
...
//				Debug.Log("ExecuteCommand: " + Event.current.commandName);
			...
			if(newMaterial != material)
			{
				for (int i = 0; i < selectedSourcePolygons.Count; i++) 
				{
					ChangePolygonMaterial(selectedSourcePolygons[i], newMaterial, Color.white);
				}
			}				
		}
		...
		void DrawExcludeBox()
		{
			...
			Rect rect = new Rect(138, 68, 60, 15);
			...
			if(SabreGUILayout.DrawUVField(rect, eastScale, ref uScaleString, "uScaleField", textFieldStyle1))
			{
				...
				{					
					for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++) 
					{
						...
						TransformUVs(polygon, UVUtility.ScaleUV, new UVUtility.TransformData(new Vector2(newEastScale/originalEastScale,1),0));
					}
				}
			}
			...
			if(SabreGUILayout.DrawUVField(rect, northScale, ref vScaleString, "vScaleField", textFieldStyle1))
			{
				...
				{					
					for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++) 
					{
						...
						TransformUVs(polygon, UVUtility.ScaleUV, new UVUtility.TransformData(new Vector2(1, newNorthScale/originalNorthScale),0));
					}
				}
			}
			...
			if(SabreGUILayout.DrawUVField(rect, eastOffset, ref uOffsetString, "uOffsetField", textFieldStyle2))
			{
				...
				{					
					for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++) 
					{
						...
						TransformUVs(polygon, UVUtility.TranslateUV, new UVUtility.TransformData(new Vector2(newEastOffset-originalEastOffset,0),0));
					}
				}
			}
			...
			if(SabreGUILayout.DrawUVField(rect, northOffset, ref vOffsetString, "vOffsetField", textFieldStyle2))
			{
				...
				{					
					for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++) 
					{
						...
						TransformUVs(polygon, UVUtility.TranslateUV, new UVUtility.TransformData(new Vector2(0, newNorthOffset-originalNorthOffset),0));
					}
				}
			}
		}
...
//				}
			...
			if(GUILayout.Button("Flatten", EditorStyles.miniButton))
			{
				...
				for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++) 
				{
					Polygon polygon = selectedSourcePolygons[polygonIndex];
					Brush brush = matchedBrushes[polygon];
					SurfaceUtility.FacetPolygon(polygon, brush.GetPolygons());
...
//			GUILayout.Space(8);
			...
			if(GUILayout.Button("Scale", EditorStyles.miniButton) || keyboardEnter)
			{
				...
				if(StringHelper.TryParseScale(scaleAmount, out rescaleVector2))
				{
					...
					{
						TransformUVs(UVUtility.ScaleUV, new UVUtility.TransformData(rescaleVector2,0));
					}
				}
			}
		...
		void ExtrudeBrushesFromSelection()
		{
			...
			for (int i = 0; i < selectedSourcePolygons.Count; i++) 
			{
				Quaternion rotation;
				Polygon[] polygons;
				SurfaceUtility.ExtrudePolygon(selectedSourcePolygons[i], out polygons, out rotation);
...
//			}
		...
		void ChangePolygonMaterial(Polygon polygon, Material destinationMaterial, Color32 destinationColor)
		{
